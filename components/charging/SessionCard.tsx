    ...
    <Card style={styles.card}>
      <View style={styles.header}>
        ...
        <View style={[styles.statusBadge, { backgroundColor: getStatusColor(session.status) + "20" }]}>
          <Text style={[styles.statusText, { color: getStatusColor(session.status) }]}>{getStatusText(session.status)}</Text>
        </View>
      </View>

      <View style={styles.addressRow}>
        <MapPin size={16} color="#6B7280" />
        <Text style={styles.address}>{session.stationAddress}</Text>
      </View>
      ...
      <View style={styles.infoGrid}>
        ...
        <View style={styles.infoItem}>
          ...
          <View style={styles.infoTextContainer}>
            ...
            <Text style={styles.infoValue}>{session.energyConsumed.toFixed(1)} кВт⋅ч</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.footer}>
        <View style={styles.durationContainer}>
          <Text style={styles.duration}>Длительность: {getDuration()}</Text>
        </View>
